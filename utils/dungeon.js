...
export const setLb = (lbCount) => {
  lb = lbCount;
};
export const getLb = () => {
  return lb;
};
export const setSpray = (iceSprayHit) => {
  icespray = iceSprayHit;
};
export const getSpray = () => {
  return icespray;
};
...
export const getCurrentClass = (name) => {
  ...
  const partyNumLine = tabNames.find((name) =>
    name.includes('§r§b§lParty §r§f(')
  );
  let currentClass = '';
  try {
    if (partyNumLine) {
      ...
      for (let i = 0; i < partyNum; i++) {
        if (
          tabNames[i * 4 + 1]
            .removeFormatting()
            .includes(!name ? Player.getName() : name)
        ) {
          currentClass = tabNames[i * 4 + 1]
            .removeFormatting()
            .match(/\((\S+)\s+(\S+)\)/)[1];
        }
      }
