...
export const getCurrentVersion = () => {
  const metadata = FileLib.read(
    './config/ChatTriggers/modules/TakeshiAddons/metadata.json'
  );
  if (metadata === '') {
...
export const printHelp = () => {
  ...
  ChatLib.chat(
    '&7|  &c"/scc"&7: &aPrint your scoreboard to chat so that you can copy it.'
  );
  ...
  ChatLib.chat(
    '&7   &aYou can bind a key for sending party chat about estimated debuff result on dragons.'
  );
};
...
export const printChangelog = (version) => {
  request({
    ...
    .then((res) => {
      ...
      if (!changelogs) {
        console.log(
          `[Takeshi] Failed to fetch changelog for version ${version}`
        );
        return;
      }
...
const checkForUpdate = () => {
  request({
    ...
    if (isNewerVersion(latestVersion, getCurrentVersion())) {
      ChatLib.chat(
        `${CHAT_PREFIX} &bA new version of TakeshiAddons (v${latestVersion}) is available!`
      );
      ChatLib.chat(
        new TextComponent(` &eClick here to update!`).setClick(
          'run_command',
          `/takeshi forceupdate`
        )
      );
      ...
      ChatLib.chat(
        `${CHAT_PREFIX} &bYou are running the latest version of TakeshiAddons!`
      );
    }
  });
};
...
export const update = (manual = false) => {
  ...
  try {
    ...
    FileUtilities.urlToFile(
      downloadUrl,
      './config/ChatTriggers/modules/TakeshiAddons.zip',
      1000,
      1000
    );
  } catch (e) {
    ChatLib.chat(
      `${CHAT_PREFIX} &c[ERROR]Connection timed out while downloading update.`
    );
    return;
  }
  setTimeout(() => {
    FileLib.unzip(
      './config/ChatTriggers/modules/TakeshiAddons.zip',
      './config/ChatTriggers/modules/'
    );
    const success = FileUtilities.delete(
      './config/ChatTriggers/modules/TakeshiAddons.zip'
    );
    if (success) {
      ChatLib.chat(
        new TextComponent(
          `&aSuccessfully updated TakeshiAddons! Click here to complete!`
        ).setClick('run_command', `/ct load`)
      );
    } else {
      ChatLib.chat(
        `${CHAT_PREFIX} &c[ERROR]Failed to extract downloaded zip file :(`
      );
    }
