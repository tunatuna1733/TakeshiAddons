...
registerWhen(register('renderWorld', () => {
    ...
    World.getAllEntitiesOfType(EntityArmorStand.class).forEach((armorStand) => {
        ...
        // blue blazes
        else if (armorStand.getName().removeFormatting().includes('[Lv150] Ashfang Acolyte')) {
            RenderLib.drawEspBox(
                armorStand.getX(),
                armorStand.getY() - 2,
                armorStand.getZ(),
        ...
        // red blazes
        else if (armorStand.getName().removeFormatting().includes('[Lv150] Ashfang Underling')) {
            RenderLib.drawEspBox(
                armorStand.getX(),
                armorStand.getY() - 2,
                armorStand.getZ(),
...
}), () => (getCurrentArea() === 'Crimson Isle' && settings.ashfanghelper), { type: 'renderWorld', name: 'Ashfang Helper' });