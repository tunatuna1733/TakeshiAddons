  ...
  categories.forEach((c) => {
    if (
      (c.category.match(/^§r§[0-9a-f]§l.+\:/) &&
        c.category.match(/^§r§[0-9a-f]§l(.+)\:/)[1].removeFormatting() ===
        name) ||
      (c.category.match(/^§r§l§r§[0-9a-f]§l.+\:/) &&
        c.category.match(/^§r§l§r§[0-9a-f]§l(.+)\:/)[1].removeFormatting() ===
        name)
    ) {
      text = c.category + '\n' + c.details.join('\n');
    }
  ...
  customHudsData.data.forEach((h) => {
    if (settings.tablistbackground) {
      const hud = new Hud(
        ...
        true,
        true,
        settings.tablistbackgroundcolor.getRGB()
      );
...
register('step', () => {
  ...
  TabList.getNames().forEach((n, i) => {
    if (n.match(/^§r§[0-9a-f]§l.+\:/)) {
      ...
      currentCategory = n;
      categoryTexts.push(
        n.match(/^§r§l§r§[0-9a-f]§l(.+)\:/)[1].removeFormatting()
      );
...
export const addCustomHud = () => {
  ChatLib.chat(
    `${CHAT_PREFIX} &aPlease select the tablist widget you want to add as a hud.`
  );
  categoryTexts.forEach((c) => {
    ChatLib.chat(
      new TextComponent(` &e${c}`).setClick(
        'run_command',
        `/takeshiaddcustomtablisthud ${c}`
      )
    );
  });
};
...
export const removeCustomHud = () => {
  ChatLib.chat(
    `${CHAT_PREFIX} &aPlease select the hud you want to &c&lremove&r&a.`
  );
  customHudsData.data.forEach((h) => {
    ChatLib.chat(
      new TextComponent(` &e${h.name}`).setClick(
        'run_command',
        `/takeshiremovecustomtablisthud ${h.name}`
      )
    );
  });
};
...
register('command', (...args) => {
  ...
  const categoryName = args.join(' ');
  customHudsData.data = customHudsData.data.filter(
