      ...
      try {
        ...
        lores.forEach((lore) => {
          if (lore.removeFormatting().includes('Tiered Bonus: Dominus')) {
            ...
            lore.removeFormatting().includes('Tiered Bonus: Hydra Strike')
          ) {
...
registerWhen(
  register('actionBar', (msg) => {
    ...
    if (crimsonResult) {
      ...
      const newCrimsonStack = parseInt(
        crimsonResult[0].removeFormatting().match(/1?[0-9]/)[0]
      );
    ...
    if (terrorResult) {
      ...
      const newTerrorStack = parseInt(
        terrorResult[0].removeFormatting().match(/1?[0-9]/)[0]
      );
...
registerWhen(
  register('soundPlay', (pos, name, vol, pitch, category, e) => {
    if (
...
registerWhen(
  register('renderOverlay', () => {
    let crimsonSecLeft = (
      (crimsonExpireSec * 1000 - (Date.now() - lastCrimsonHit)) /
      1000.0
    ).toFixed(1);
...
registerWhen(
  register('renderOverlay', () => {
    let terrorSecLeft = (
      (terrorExpireSec * 1000 - (Date.now() - lastTerrorHit)) /
      1000.0
    ).toFixed(1);
