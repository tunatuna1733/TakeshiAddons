...
const hypixelPing = new C17PacketCustomPayload(
  'hypixel:ping',
  new PacketBuffer(Unpooled.buffer(1).writeByte(1))
);
let time = 0;
let ping = 0;
...
registerWhen(
  ...
  () => settings.pinghud,
  { type: 'renderOverlay', name: 'Ping Hud' }
);
...
const drawPingBox = (topX, topY) => {
  const x = topX + 30;
  const y = topY + 5;
  Renderer.drawLine(Color.WHITE.getRGB(), x, y, x + width, y, 1);
  Renderer.drawLine(
    Color.WHITE.getRGB(),
    x + width,
    y,
  ...
  Renderer.drawLine(Color.WHITE.getRGB(), x, y, x, y + height, 1);
  const [minLimit, maxLimit] = calcMinAndMax(
    Math.min(...pings),
    Math.max(...pings)
  );
...
register('command', () => {
  ...
  ChatLib.chat(
    `${Math.min(...pings)} -> ${minLimit}, ${Math.max(...pings)} -> ${maxLimit}`
  );
