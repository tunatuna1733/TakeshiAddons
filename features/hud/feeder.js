            ...
            if (item) {
                if (item.getName().removeFormatting() === 'Use Caducous Feeder') {
                    ...
                    inventory.getItems().forEach((i) => {
                        if (i && i.getName().removeFormatting() === 'Ultimate Carrot Candy') {
                            carrotCount++;
                        }
                    });
                    if (carrotCount === 1) {
...
}), () => settings.feedertimer, { type: 'guiMouseClick', name: 'Feeder Timer' });
...
registerWhen(register('renderOverlay', () => {
    ...
    const latest = Math.min(data['feeder']['times'][0], data['feeder']['times'][1]);
    const diff = Date.now() - latest;
    if (diff > 60 * 60 * 20 * 1000) {
        feederHud.draw('&aFeeder: &6Ready');
    } else {
        const remainingTime = (60 * 60 * 20 * 1000 - diff) / 1000;
        feederHud.draw(`&aFeeder: &6${numeral(remainingTime).format('00:00:00')}`);
    }
}), () => settings.feedertimer, { type: 'renderOverlay', name: 'Feeder Timer' });
