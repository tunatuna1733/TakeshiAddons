  ...
  register('renderOverlay', () => {
    ...
    if (isDropshipApproaching && Player.getY() > 20) {
      if (Date.now() - dropshipTimer > explosionTime) {
        ...
        dropshipHud.draw(
          `&aBomb drops in &6${((explosionTime - (Date.now() - dropshipTimer)) / 1000).toFixed(1)}s`
        );
        if (Date.now() - dropshipTimer > explosionTime - warnTime && !warned) {
          World.playSound('note.pling', 4, 1.5);
          Client.showTitle(
            `&cBomb Drop in ${settings.dropshiptime} secs!`,
            '',
            0,
            3 * 20,
            0
          );
          if (settings.senddropshipwarning)
            ChatLib.command(`pc Bomb drops in ${settings.dropshiptime} secs!`);
          warned = true;
        }
      }
    }
...
registerWhen(
  register('chat', () => {
    ...
    "[NPC] Elle: A Dropship is approaching! Take it down before it's too late!"
  ),
  () => settings.dropship && getCurrentArea().includes('Kuudra'),
...
registerWhen(
  register('chat', () => {
    ...
    "[NPC] Elle: A fleet of Dropships are approaching! Take them down before it's too late!"
  ),
  () => settings.dropship && getCurrentArea().includes('Kuudra'),
  { type: 'chat', name: moduleName }
);
...
registerWhen(
  register('chat', () => {
    isDropshipApproaching = false;
    warned = false;
  }).setCriteria('The Dropship Bomb hit you for ${dmg} true damage.'),
  () => settings.dropship && getCurrentArea().includes('Kuudra'),
  { type: 'chat', name: moduleName }
);
