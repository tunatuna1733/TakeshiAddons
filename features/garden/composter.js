...
register('tick', () => {
    ...
    tablistEntries.forEach((line) => {
        ...
        if (isComposterLine) {
            if (line.removeFormatting().includes('Organic Matter:')) {
                const shortNum = line.removeFormatting().replace('Organic Matter: ', '');
    ...
    const organicMatterRemaining = Math.floor(organicMatter / organicMatterRequired);
    ...
    gardenData.endTime = organicMatterEnd > fuelEnd ? fuelEnd + Date.now() / 1000 : organicMatterEnd + Date.now() / 1000;
    gardenData.save();
});
...
register('postGuiRender', () => {
    ...
    if (!guiOpen && inventory && inventory.getName() === 'Composter Upgrades') {
        ...
        const guiLoaded = register('tick', () => {
            ...
            if (!speedUpgradeItem || !fuelUpgradeItem || !organicUpgradeItem || !costUpgradeItem) return;
            if (!speedUpgradeItem.getName().includes('Composter Speed') ||
            ...
            const speedUpgrade = decodeNumeral(speedUpgradeItem.getName().removeFormatting().replace('Composter Speed ', ''));
