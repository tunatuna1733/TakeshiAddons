...
registerWhen(
  register('step', () => {
    if (!isNecronLow)
      isNecronLow =
        inNecron() &&
        getBossHealthPercent() < 0.5 &&
        getBossHealthPercent() > 0;
  }).setDelay(1),
...
registerWhen(
  ...
  }).setChatCriteria('${player} picked the Corrupted ${color} Relic!'),
  () => settings.relicwaypoint && getCurrentArea() === 'The Catacombs (M7)',
...
register('command', () => {
  ChatLib.chat(
    `${CHAT_PREFIX} isNecronLow: ${isNecronLow}, isRelicPicked: ${isRelicPicked}`
  );
