...
registerWhen(
  register('clicked', (x, y, button, isDown) => {
    ...
    if (
      heltItemId === 'ICE_SPRAY_WAND' &&
      !isIceSprayUsed &&
      Date.now() - iceSprayTime > 5 * 1000
    ) {
      isIceSprayUsed = true;
      iceSprayTime = Date.now();
    }
  }),
  () => settings.icespray && inM7(),
  { type: 'clicked', name: moduleName }
);
...
registerWhen(
  register('renderWorld', () => {
    ...
    if (iceDetected) {
      World.getAllEntitiesOfType(
        Java.type('net.minecraft.entity.boss.EntityDragon').class
      ).forEach((dragon) => {
