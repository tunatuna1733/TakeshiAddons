...
const isCoordValid = (coord) => {
  const flooredFloat =
    Math.floor(
      (coord -
        Math.trunc(Math.ceil(coord * Math.pow(10, 4)) / Math.pow(10, 4))) *
        Math.pow(10, 4)
    ) / Math.pow(10, 4);
...
registerWhen(
  ...
  () => settings.chestglitch && (inM7() || inF7()),
...
registerWhen(
  ...
  () => settings.chestglitch,
  { type: 'worldUnload', name: moduleName }
);

register('command', () => {
  ChatLib.chat(
    `${CHAT_PREFIX} &e[DEBUG] x: ${isCoordValid(Player.getX())}, z: ${isCoordValid(Player.getZ())}`
  );
