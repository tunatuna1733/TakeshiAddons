...
registerWhen(
  ...
  () => settings.mineshaftmobbox && getCurrentArea() === 'Mineshaft',
  { type: 'renderWorld', name: 'Mineshaft Mob Box' },
);
...
registerWhen(
  ...
  }).setChatCriteria('WOW! You found a Glacite Mineshaft portal!'),
  () => settings.mineshaftmobbox && getCurrentArea() === 'Dwarven Mines',
  { type: 'chat', name: 'Mineshaft Notification' },
);
    ...
    World.getAllEntitiesOfType(EntityArmorStand.class).forEach((e) => {
      ...
      if (headItem) {
        ...
        if (variant !== '') {
          ...
          Tessellator.drawString(variant, x, y, z, color.getRGB());
        }
      }
    });
  }),
  () => settings.corpselocation && getCurrentArea() === 'Mineshaft',
  { type: 'renderWorld', name: 'Show corpses location' },
);
