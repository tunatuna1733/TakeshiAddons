...
const checkHealth = (entity, healths) => {
  ...
  healths.forEach((h) => {
    if (
      entity.getEntity().func_110143_aJ() === h * derpyMult || // getHealth
    ...
    if (entity.getEntity() instanceof EntityZombie) {
      ...
      filtered.forEach((m) => {
        if (checkIsland(m.island)) {
          if (m.villager === entity.getEntity().func_82231_m()) {
            // isVillager
            if ('armor' in m) {
              if (m.armor) {
                if (
      ...
      filtered.forEach((m) => {
        if (checkIsland(m.island)) {
          if (m.skeletonType === entity.getEntity().func_82202_m()) {
            // getSkeletonType
            if ('isInvisible' in m) {
              if (m.isInvisible === entity.getEntity().func_82150_aj()) {
                // isInvisible
                drawList.push(entity);
              }
            } else {
...
registerWhen(
  register('renderWorld', () => {
    drawList.forEach((e) => {
      if (e && !(e.isDead() || e.getEntity().func_110143_aJ() <= 0)) drawBox(e);
    });
  }),
  () => settings.mobbox,
