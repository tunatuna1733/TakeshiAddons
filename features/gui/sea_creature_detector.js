...
registerWhen(register('renderWorld', () => {
    ...
    shouldRender = (!settings.fishingrodstoprender || (settings.fishingrodstoprender && !(holdingItem && holdingItem.getID() === 346)));
    mobs = 0;
...
registerWhen(register('worldUnload', () => {
    mobs = 0;
}), () => settings.seacreature, { type: 'worldUnload', name: 'SeaCreature Detect' });

registerWhen(register('renderOverlay', () => {
    seaCreatureCounterHud.draw(`&6Sea Creatures: &a${mobs}`);
}), () => settings.seacreaturecounter && getCurrentArea() === 'Crimson Isle', { type: 'renderOverlay', name: 'SeaCreature Counter' });
