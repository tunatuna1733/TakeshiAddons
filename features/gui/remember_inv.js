...
const getSkullTextureId = (nbt) => {
    const encoded = nbt.getCompoundTag('tag')
...
const getTexture = (textureId) => {
    if (!Object.keys(textures).includes(textureId)) {
        new Thread(() => {
            textures[textureId] = new Image(ImageIO.read(new URL(`https://mc-heads.net/head/${textureId}`).openConnection().inputStream));
        }).start();
    }
...
register('command', (...args) => {
    ...
    inventoryData.save();
}).setCommandName('rememberinventory', true).setAliases(['ri']);
...
register('renderSlot', (slot, gui, e) => {
    if (Java.type('net.minecraft.client.gui.inventory.GuiInventory').class.isInstance(gui) && Object.keys(inventoryData).includes(selectedOption)) {
