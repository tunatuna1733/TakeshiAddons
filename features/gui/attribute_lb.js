const lbKey = new KeyBind(
  'View Lowest bin with attribute considered',
  Keyboard.KEY_K,
  'TakeshiAddons'
);
...
const createLowestBINCard = (
  ...
  if (!auctionData.original && !auctionData.type) {
    ...
    if (auctionData.type) {
      ...
      const lastUpdatedTime = getMinAndSec(
        Date.now() - auctionData.type.lastUpdated
      );
      new UIText(
        `Last Update: ${lastUpdatedTime[0]}m${lastUpdatedTime[1]}secs ago`
      )
        .setX((50).pixels())
    ...
    if (auctionData.original) {
      ...
      const lastUpdatedTime = getMinAndSec(
        Date.now() - auctionData.original.lastUpdated
      );
      new UIText(
        `Last Update: ${lastUpdatedTime[0]}m${lastUpdatedTime[1]}secs ago`
      )
...
const createAuctionGui = (item) => {
  ...
  if (armorType !== 'Unknown') {
    // armor
    createLowestBINCard(
      onlyFirstAuctionCard,
      {
        original: results[1][attributeId1][0],
        type: results[0][attributeId1][0],
      },
      attributeName1,
      attributeLevel1,
      armorType,
    ...
    if (attributeId2) {
      createLowestBINCard(
        onlySecondAuctionCard,
        {
          original: results[1][attributeId2][0],
          type: results[0][attributeId2][0],
        },
        attributeName2,
        attributeLevel2,
        armorType,
...
register('renderOverlay', () => {
  if (lbKey.isPressed() && Player.getHeldItem()) {
    if (
      Player.getHeldItem()
        .getNBT()
        ?.get('tag')
      ...
      ChatLib.command(
        `ahs ${getPureItemName(getItemId(Player.getHeldItem()))}`
      );
    }
  }
  if (guiOpen) {
